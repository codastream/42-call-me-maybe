...
class ParamMatcher(TokenMatcher):
    ...
    def __init__(self, function_def: FunctionDefinition, close_suffix: bytes = b'}}') -> None:
        ...
        self.sub_state = MatcherState.EXPECT_DYNAMIC_PARAM_KEY
    ...
    def _allowed_keys(self) -> list[bytes]:
        ...
        return [prefix + f'"{k}": '.encode() for k in self.all_keys if k not in self.evaluated_params]
    ...
    def _remaining_key_targets(self, exclude_current: bool = True) -> list[bytes]:
        ...
        return [b', ' + f'"{k}": '.encode() for k in keys]
    ...
    def evaluate(self, buf: bytes) -> Any | bool:
        if self.sub_state == MatcherState.EXPECT_DYNAMIC_PARAM_KEY:
    ...
    def is_complete(self, buf: bytes) -> bool:
        ...
        if self.sub_state != MatcherState.EXPECT_VALUE_PARAM_VAL:
            return False
    ...
    def commit(self, buf: bytes) -> bytes:
        ...
        if self.sub_state == MatcherState.EXPECT_DYNAMIC_PARAM_KEY:
            if buf in self._allowed_keys():
                ...
                self.sub_state = MatcherState.EXPECT_VALUE_PARAM_VAL
            return b""
    ...
    def display_state(self) -> str:
        return f"✅ evaluated params = {self.evaluated_params}\n"
